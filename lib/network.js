 ...
 *   1. Add the USB LAN base interface directly to br0 (NO VLAN sub-interfaces)
 *      VLAN 22/13 are only on WAN interface to avoid conflicts.
 *   2. Persist to DB
 */
  ...
  try {
    ...
    if (existingBridge) {
      ...
      if (modified) {
        ...
        console.log(`[USB-LAN] ${usbIface} already in ${bridgeName}. No changes needed.`);
      }
    } else {
      // Bridge doesn't exist yet — create it with USB LAN + existing LAN candidates
      const interfaces = await getInterfaces();
      const classified = classifyInterfaces(interfaces);
    ...
    console.log(`[USB-LAN] Successfully configured ${usbIface} into bridge ${bridgeName} (no VLANs).`);
  } catch (e) {
...
async function startUsbLanMonitor() {
  ...
  // Periodic scan for newly plugged USB LAN adapters (every 30s to save CPU)
  if (usbLanMonitorInterval) clearInterval(usbLanMonitorInterval);
  usbLanMonitorInterval = setInterval(scanAndConfigureUsbLan, 30000);
  console.log('[USB-LAN] Monitoring for new USB-to-LAN adapters every 30s.');
  }
...
// Cache for scanWifiDevices — prevent hammering journalctl/iw on rapid calls
let _scanWifiCache = null;
let _scanWifiCacheTs = 0;
const SCAN_WIFI_CACHE_TTL = 15000; // 15 seconds

async function scanWifiDevices() {
  // Return cached result if still fresh
  if (_scanWifiCache && (Date.now() - _scanWifiCacheTs) < SCAN_WIFI_CACHE_TTL) {
    return _scanWifiCache;
  }
  ...
  try {
    const hostnamesFromLogs = await (async () => {
      const sources = [];
      try {
        const { stdout } = await execPromise('journalctl -u dnsmasq -n 100 --no-pager');
        sources.push(stdout);
      } catch (e) {}
    ...
    _scanWifiCache = devices;
    _scanWifiCacheTs = Date.now();
    return devices;
  } catch (err) {
